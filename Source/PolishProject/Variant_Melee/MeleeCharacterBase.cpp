...
AMeleeCharacterBase::AMeleeCharacterBase()
{
	...
	KnockbackComponent = CreateDefaultSubobject<UC_KnockbackComponent>(TEXT("KnockbackComponent"));
...
{
	KnockbackComponent->Apply(Direction, ChainDepth);
}

void AMeleeCharacterBase::OnDeath_Implementation()
{
	KnockbackComponent->Cancel();
